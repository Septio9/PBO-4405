...
public class ProductSearchController implements Initializable {
    ...
    void update(ActionEvent event) {
        ...
        if (flagAdd){
            ...
            try {
                PreparedStatement st = conn.prepareStatement(sql);st.setString(1,tfkode.getText());
...
//do something..
            ...
            try {
                PreparedStatement st = conn.prepareStatement(sql);st.setString(1,tfNama.getText());
    ...
    void setFilter(){
        ...
        tfKeyword.textProperty().addListener((observable,oldValue,newValue)->{
            filterData.setPredicate(Produk->{
                ...
                (Produk.getKode().toLowerCase().indexOf(searchKeyword)>-1){
                    return true;
                }else
                    return false;
            });
        });
        SortedList<Produk> sortedData = new SortedList<>(filterData);sortedData.comparatorProperty().bind(tbProduk.comparatorProperty());tbProduk.setItems(sortedData);
    }
    @Override
